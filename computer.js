exports.run = (Discord, client, message, args, sql, prefix) => {
  ...
  if (args.length > 1)
	{
			message.channel.send ('`Error: More than 1 argument. Usage: ' + prefix + 'computer <@user>`');
			return;
	}
  ...
  sql.get(`SELECT * FROM userData WHERE userID = ${defineduser.id}`).then(row => {
    ...
    sql.get(`SELECT * FROM compData WHERE computerName = "${row.Computer}"`).then(row1 => {
      embed.setAuthor(defineduser.username + "'s Computer", row1.logoURL, "");
      embed.setThumbnail (row1.imageURL);
      embed.addField ("Owner:", defineduser.username + "#" + defineduser.discriminator, true);
      embed.addField ("Computer:", row1.computerName, true);
      embed.addField ("Boost:", row1.baseBoost + "%", true);
