...
  			return (num + '').replace(/(\d)(?=(\d{3})+$)/g, '$1,');
  ...
  if (args.length > 1)
	{
			message.channel.send ('`Error: More than 1 argument. Usage: ' + prefix + 'currency <@user>`');
			return;
	}
  ...
  sql.get(`SELECT * FROM userData WHERE userID = ${defineduser.id}`).then(row => {
    ...
    embed.setAuthor(defineduser.username + " Cryptocurrency",  defineduser.displayAvatarURL, "");
    embed.addField ("Currency:", "**" + commafy (currency) + "** :diamond_shape_with_a_dot_inside:", true);
